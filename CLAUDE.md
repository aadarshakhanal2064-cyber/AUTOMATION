...
| Provisional Statement | Automation Hub | `provisionalStatement.js` + `provisionalStatementEngine.js` *(output via `finStatementExport.js`)* | `ps-` | *(none yet — derived per run)* | [provisional-statement](docs/modules/provisional-statement.md) |
...
- **Clickjacking has two layers**: `X-Frame-Options: DENY` from `vercel.json` on the real host, plus a framebuster script at the top of `<body>` that also covers the Pages mirror. Note `frame-ancestors` is **stripped by spec from any CSP delivered in a `<meta>` tag**, so putting it there reads as a fix and protects nothing; `Referrer-Policy` *is* honoured as a meta and is set for the mirror's benefit.
...
- **Service Memo's Audit Fees list fires on ANY verified ARF track, not full completion, and now also feeds from Projection Report** (2026-08-10, extended 2026-08-15, user decisions) — the moment a client's IT Return, Estimate Return *or* Tax Clearance is first verified in Audit Report Finalization, one fee-due item appears for that client+FY (showing which tracks are done), rather than waiting for the whole filing (IT+Estimate, or IT+Estimate+Tax for a D-3) to finish. Derived the same way as Work Done's Pending List — reads `audit_report_finalization` directly, nothing stored twice — and a group drops off once an **Audit / Statutory Audit** memo exists for that client+FY (matched via `NepaliLocale.fyStartYear()` across ARF's slash format and Service Memo's dash format). **A saved Projection Report is a second source, added 2026-08-15**: `smFeeDueRows()` (renamed from `smPendingAuditRows()`) also unions rows from `projection_reports`, one per `(client, fiscal_year_base)`, dropping off once a **Bank Loan Related / Provisional/Projected** memo exists for that client+FY. Because Projection's own Updation mode always writes to the SAME `projection_reports` row (§15 below), deriving straight from that table is what keeps a re-run or an update from ever appearing twice — no save-event flag needed. The tab/button was renamed **"Pending Audit Fees" → "Audit Fees" → "Pending Memos"** over 2026-08-10/15, since the list stopped being audit-only. `smOpenCreateFromPending()` now also prefills a **description** for both sources so only the Professional Fee is left blank. `SM_FY_DEFAULT` (via `smFyLabel(window.FY_DEFAULT_START)`) replaced the old `NepaliLocale.todayBs()`-derived default, deliberately matching ARF's `ARF_FY_DEFAULT` — a memo written in Shrawan is routinely for the year just closed, not the one that just started. Selecting **Statutory Audit** caps the FY datalist at `SM_FY_AUDIT_CAP` (2082) and below and fills the field **only if blank**, never overwriting a typed or prefilled year. Don't change the trigger to "full completion only", make the FY auto-fill unconditional, or add a dedicated `SERVICE_MEMO_TASKS` entry for projection fees without asking (Bank Loan Related / Provisional/Projected was a deliberate reuse of an existing slot, not a new one).
...
- **Provisional Statement derives; Audited Statement lays out — and the solve directions are OPPOSITE** (2026-08-19, user ask) — `finStatementEngine` takes a profit target and plugs Purchases; `provisionalStatementEngine` takes Purchases as typed and lets profit fall out. That is the whole difference between reconstructing a finished year and projecting one, and it is why they are two engines rather than one with a flag. Every rule in the provisional engine is quoted from the firm's own `Pashupati Marvel Pvt Ltd 82.83 Provisional.xlsx` with the source formula in a comment beside it, and `node tools/psVerify.mjs` proves all 77 of them against Excel's own cached results. Don't "simplify" the two engines together.
...
| `tools/psVerify.mjs` | Run, not read | Provisional Statement harness — `node tools/psVerify.mjs`. Replays the firm's own Pashupati Marvel 82.83 workbook through the engine and asserts 77 derived figures against Excel's own cached results. **Run it before and after any change to the derivation rules.** |
