...
- **Projection's IRD Gross Income is turnover, Paid up Capital is share capital alone, and Loan from Bank is every facility** (2026-08-11, user decision) — Gross Income was gross *profit*, Paid up Capital added the solver's Additional Capital (which is not issued capital), and the loan row omitted `longTermLoan` (where hire purchase sits) while counting `directorLending` (related-party, not a bank). Each row's audited value, projected value **and** Excel cross-sheet formula must change together or the sheet drifts.
