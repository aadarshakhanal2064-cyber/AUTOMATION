...
- **`vercel.json`'s headers are NOT active and never have been.** GitHub Pages does not read that file — verified against the live response, which carries only GitHub's own HSTS. The file is kept as a ready-made config for the Stage 5 hosting move, not because it does anything today. **Don't cite it as a control.**
- **Clickjacking is handled by a framebuster script at the top of `<body>`**, because on a static host nothing else can be. `X-Frame-Options` is header-only, and `frame-ancestors` is **stripped by spec from any CSP delivered in a `<meta>` tag** — putting it there reads as a fix and protects nothing. `Referrer-Policy` *is* honoured as a meta and is set.
...
| Security headers cannot be sent from GitHub Pages | Medium | `X-Frame-Options`, `X-Content-Type-Options` and `Permissions-Policy` are header-only and a static host cannot send them. Mitigated by the framebuster and the meta `Referrer-Policy` (§13); the real fix is the Stage 5 hosting move, for which `vercel.json` is already written. |
