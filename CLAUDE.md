...
| Work Done | Sidebar | `workDone.js` | `wd-` | `work_done` *(+ reads `document_register`)* | [work-done](docs/modules/work-done.md) |
...
- **`WD_WORK_TYPES[].fileLabel` must stay spelled identically to the matching `FM_DOC_TYPES[].label`** — `document_register.doc_types[].type` stores the label *text*, not the key, so renaming a File In Out document type would silently empty the Pending List. `wdBrokenFileLabels()` exists to make that visible; don't remove it.
- **Work Done joins fiscal years on `NepaliLocale.fyStartYear()`, never string equality** — File In Out's FY is free text in dash format (`2081-82`) and Work Done's is a dropdown in slash format (`2082/83`). §8's per-module formats stay as they are; the join normalizes at the boundary. A string join matches nothing and fails *silently*, which is why unparseable years are counted and reported instead of dropped.
...
- **Work Done's rows are 3-state (Not Started / In Progress / Done), not a done-tick** (2026-08-09, user decision) — "In Progress" is what stops two staff starting the same job, which is half the reason the module exists. A 4th `not_applicable` state was offered and declined; the known consequence is that "Complete" rarely fires, and that's accepted because the Pending List is the module's primary signal.
- **Work Done merges newly-added work types into existing records on drawer load (`wdMergeItems`), unlike Audit Checklist which loads verbatim** — a tracker whose config list can grow would otherwise never show new work on any existing record, reintroducing the exact "did anyone do this?" problem it was built to solve. The merge is strictly additive: nothing stored is altered or dropped.
