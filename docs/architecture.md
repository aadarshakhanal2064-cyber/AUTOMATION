...
2. `window.sb.auth.onAuthStateChange()` fires (`INITIAL_SESSION` / `SIGNED_IN` / `SIGNED_OUT`) and routes into `afterSupabaseSignIn(session)`. `INITIAL_SESSION` is what restores a session across a page reload. `SIGNED_OUT` is skipped while the Access Denied screen is up, because that sign-out is the app's own. **`SIGNED_IN` is guarded since 2026-08-21 (Stage 2):** supabase-js re-emits it on tab focus/session recovery, and unguarded it re-ran the entire boot chain — the edge logs showed ~310 full boots/day for ≤8 staff. If `window.currentUser` already matches the session's email, the event is a no-op; a real sign-out→sign-in passes because `signOut()` nulls `currentUser`. The boot itself runs at DOM-ready (auth.js is deferred and last), no longer on window `load`.
3. `afterSupabaseSignIn()` looks the email up in `org_members` with **`.ilike()`** — `private.jwt_email()` lowercases before matching, so `.eq()` would reject a mixed-case address that RLS accepts. Not found → Access Denied **and** `sb.auth.signOut()`, so a rejected user keeps no session. Found → `window.currentUser = {email, role}`, `OrgIdentity.load()` (the one genuinely serial step — letterheads must exist before any module opens), admin-only UI shown conditionally, then **one parallel wave** of `loadClients()` + `loadRegistrarCompanies()` + `loadDashboard()` (Stage 2 — these were serial stages; the dashboard's client-count stat is patched in after the client list resolves). `loadSidebarStorageUsage()` is off the boot promise entirely (`requestIdleCallback`) — it is cosmetic and was measured at 161 ms × ~300 boots/day as the boot's last serial await. A 4-minute keep-alive `HEAD` ping (visible tabs only) keeps the free-tier database from going idle, and config.js's fetch wrapper raises the `#net-slow-banner` notice when any request has been in flight over 3 s.
