...
| TrialBalanceReader | `trialBalanceReader.js` | `parse(wb, XLSX)` → `{ok, sheetName, blocks, check, sectionOf, amountOf, issues}`; `toFigures(parsed)` → `{cy, loans, ppe, tds, otherExpenses, directExtra, extraRecv, reserves, filled, check}`. Added 2026-08-30 so a statement's CURRENT year can be read off the firm's own trial balance rather than typed. **Everything is heading-driven, nothing positional beyond "column A is the label"** — the value columns are detected by which columns actually carry numbers, so a file with an extra leading column still reads. Sections are found by their numbered heading and claim the first subtotal-column value in their span, because in the reference file that subtotal sits on a row labelled "Land" while holding the whole PPE figure. Blocks match on WORDS, because the letter "B" labels both Expenses and Equity and Liabilities. `TERMINATORS` stops the walk at the sheet's own proof block, without which "Total of Assets & Expenses" folded into the last open section. Detail-line vocabularies (`REVENUE_LINES`, `FINANCE_LINES`, `LOAN_GROUPS`, `PPE_CLASSES`, `TDS_LINES`) are ordered most-specific-first, the `SPB_HEADER_RULES` idiom — "Interest Expenses on HP/Term loan" and "…on OD/CC/Demand/ST" differ only in their tail, and a plain "Working Capital Loan" must not be caught by the Permanent-WC pattern (CLAUDE.md §15). An unrecognised head is **kept under its own name, never dropped**. Node-loadable; `tools/tbVerify.mjs` carries a 30-line SheetJS stand-in so the harness adds no dependency. |
| TrialBalanceModel | `trialBalanceModel.js` | `blank()` / `normalize(state)` · `totals(state)` → `{sec, blocks, debits, credits, difference, foots}` · `derive(state, opts)` → `{values, profit, totals, revenue, loans, tax, issues}` · `toReport(state, opts)` (the `TrialBalanceReader.toReport()` shape, so `fsxTbSheet` draws it) · `buildSheets(state, opts, GEOM)` → the SFP and SOI sheet models · `applyEdit(state, rowKey, value)` → `{ok, state, message, changed}`. Added 2026-08-30 for the Trial Balance module (`docs/modules/trial-balance.md`). **The whole engine hangs off one identity**: a trial balance foots exactly when the balance sheet balances, because `Assets + Expenses = Revenue + Equity and Liabilities` and `Assets = Equity and Liabilities + (Revenue − Expenses)` are the same statement rearranged. So the difference is ONE number, reported once and shown from both sides — and `derive()` recomputes the balance-sheet gap **independently** rather than copying the trial difference, deliberately redundant arithmetic that is the only place a section which stops reaching the balance sheet would show up. **`SFP_ROWS` / `SOI_ROWS` are one table driving the printed sheet, the on-screen editor AND the write-back**, so a row can never be displayed from one place and written back to another; a row's `src` says where its figure comes from and `applyEdit` reverses exactly that description. `src.kind` is `section` (1:1), `sum` (several, `primary` takes the adjustment), `reserves` (writes `value − profit`), `loans` (one side of the split), `expenses` (a section less the named tax line), `revenue` (a name-matched bucket) or `calc` (never editable). Section ids, the revenue split and the loan facility groups are **`TrialBalanceReader`'s** (which exports `REVENUE_LINES` and `classify` for this) — a typed trial balance and an imported one are the same document, and a second copy of those spellings would drift the first time a client wrote "Commission Income". `FSX_GEOM` is an ARGUMENT, not an import: this is core and must not depend on the feature layer, and it stays Node-loadable so `tools/tbsVerify.mjs` can replay a real ledger and drive every write-back. |
| NepalTax | `nepalTax.js` | `compute({returnType, location, d2Nature, entity, special, filing, turnover, taxableProfit})` → `{returnType, base, rate, tax, label, workings[], warnings[]}`, plus the tables themselves (`LOCATIONS`, `RETURN_TYPES`, `D2_NATURES`, `LADDERS`, `ENTITY_RATES`), `returnTypeFromClient(itReturnType)` and **`autoReturnType({entity, turnover, taxableProfit})`** — the statute as a decision tree (entity → 30-lakh presumptive ceiling → 1-crore / 10-lakh-income turnover-tax gate → D-3), each answer naming the threshold that decided it — a SUGGESTION helper only: `compute()` never resolves through it (the return type is the preparer's choice), it just names the type that fits inside a breach message. **`LIMITS` / `limitsFor(key)`** is the enforcement half: D-1 caps turnover at Rs 30,00,000 and taxable income at Rs 3,00,000, D-2 at Rs 1,00,00,000 and Rs 10,00,000, D-3 at neither; a breach comes back in `blocking[]` (distinct from `warnings[]`) and the statement modules refuse to generate output on it. Added 2026-08-29 for Audited Statement, from a rule sheet the firm's chartered accountant supplied, cross-checked line by line against the Income Tax Act 2058 for **F.Y. 2082-83**. **It is an engine rather than two more constants in the statement engine because two of the three rules do not read profit at all** — a D-1 charge is a flat figure decided by the client's municipality and a D-2 charge is a percentage of *turnover*, neither of which can be written inside an expression whose only variable is `taxableProfit`. Turnover bands are computed **marginally over Rs 30,00,000 on top of the location base** rather than stored as the six pre-summed figures the Act publishes, because six stored figures is how the three location tiers drift apart — `tools/taxVerify.mjs` asserts the marginal form reproduces every published one. `workings[]` is returned so a screen can *show* the arithmetic instead of asserting a figure (the CA's sheet has an "Example" column for the same reason) and so the harness can assert the workings foot to the charge. `warnings[]` fires where the RULE and the FIGURES disagree — a D-2 return over the Rs 1 crore turnover or Rs 10,00,000 income ceiling, a D-3 proprietor still inside the D-2 range — and is **shown, never silently corrected**, the same rule the statement's proof rows follow. `returnTypeFromClient` maps the directory's `'D-01'/'D-02'/'D-03'` and returns **null for `'D1/D2'`**, which genuinely means "one of the two, the preparer decides" (CLAUDE.md §15) — a prefilled guess there would be a guess nobody could see. Node-loadable; the statement engine `require`s it in Node and reads the global in the browser, so `tools/psVerify.mjs` keeps working by requiring the statement engine alone. |
