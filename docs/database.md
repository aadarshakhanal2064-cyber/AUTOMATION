...
| `audit_report_finalization` | Per-client, per-fiscal-year IT/Estimate return & tax clearance tracker (Audit Report Finalization, §5.21). `client_id` (**NOT NULL** FK → clients, **on delete restrict** — directory clients only), `client_name`/`client_pan` snapshots, `fiscal_year` (slash format), `auditor` CHECK-constrained to the 5-name `ARF_AUDITORS` list, `it_entered_by`/`it_submission_no`/`it_verified` (nullable boolean), `estimate_checked_by`/`estimate_verified` (nullable boolean), `tax_clearance` (NOT NULL boolean, default false)/`tax_clearance_date`, `remarks`. **No stored status column** — the 4-key IT/Estimate badge state is derived in JS from the columns above. Unique on `(client_id, fiscal_year)` — one evolving record, not a row per edit. Member-CRUD RLS + shared `set_updated_at`. `db/2026-08-09_audit_report_finalization.sql`. |
