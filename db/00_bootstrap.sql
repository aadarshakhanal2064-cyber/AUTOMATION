...
--
--  The dated migrations beside it are an INCREMENTAL history that starts on
--  2026-07-16. Five surviving tables predate it — app_users, clients,
--  client_shareholders, audit_log, send_logs — because they were built by hand
--  in the Supabase dashboard before the migration workflow existed, and
--  set_updated_at() is in the same state: only ever ALTERed by
--  2026-07-16_rls_lockdown, never created. (Four more such tables and two more
--  such functions existed until 2026-08-18, when Billing was removed.) Running db/*.sql in date order against an empty project
...
--
--  WHAT IT CONTAINS: 23 tables, 3 trigger functions, 3 private auth helpers,
--  1 RPC, 49 indexes, 17 triggers, 82 RLS policies, 18 table comments.
--  Counts verified against what this file emits, and against the live schema.
--
--  Billing was removed on 2026-08-18 (db/2026-08-18_drop_billing.sql), taking
--  invoices, invoice_items, invoice_payments and firm_bank_details with it.
--
...
-- ════════════════════════════════════════════════════════════════════════════
--  §2  TABLES
--  Dependency order: clients first, everything that references it after.
-- ════════════════════════════════════════════════════════════════════════════
...
comment on table public.service_memos is 'Internal service records + fee tracking (Service Memo module). Not a tax invoice; Billing, which issued those, was removed 2026-08-18, leaving this the firm''s only fee record.';
...
-- ── 9.4 Everything else: full CRUD for members ──────────────────────────────
...
-- ════════════════════════════════════════════════════════════════════════════
--  §10  SEED — the one row a new instance cannot start without
--  Commented out because the value is per-organisation. Uncomment, replace,
--  run. Everything else the app creates for itself. (A second seed row, for
--  firm_bank_details, was needed until Billing was removed on 2026-08-18.)
-- ════════════════════════════════════════════════════════════════════════════

--  The first admin. See §11 — this is only HALF of creating a login.
--
...
--
--  role is 'admin' or 'staff' and affects UI visibility plus the three
--  admin-only policies above (clients INSERT/DELETE, client_shareholders
--  INSERT). A fourth, firm_bank_details writes, went with Billing.
-- ════════════════════════════════════════════════════════════════════════════
...
--
--    tables 23 | public_functions 4 | private_helpers 3
--    extra_indexes 49 | triggers 17 | policies 82 | rls_enabled 23
--
--  public_functions is 3 trigger functions + 1 RPC. extra_indexes excludes
--  the PK/UNIQUE indexes Postgres creates from the constraints in §2.
--
--  Verified against the live project on 2026-08-18, after the Billing drop.
-- ════════════════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════════════════
--  NO KNOWN DIVERGENCE FROM THE LIVE PROJECT as of 2026-08-18.
--  The previous divergence — public.get_vat_fy_stats(text), broken since
--  vat_filings was dropped on 2026-08-10 — was itself dropped by
--  db/2026-08-18_drop_billing.sql, so this file and the live schema now agree.
-- ════════════════════════════════════════════════════════════════════════════
