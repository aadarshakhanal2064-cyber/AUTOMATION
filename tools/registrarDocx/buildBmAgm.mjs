...
//    node buildBmAgm.mjs "<path to the firm's source .docx>"
//
...
import {
  readDocx, writeDocx, splitBody, createTransformer, createPageBreakStyle, walkBody,
  swapFonts, stripHighlight, stripListBullet, mangalToNirmala, scaleFontSizes, tightDocDefaults,
  assertTokenised, swapExact, stripProofErr,
} from './core.mjs';
...
try {
  ...
  console.error(
    "Missing tools/registrarDocx/sample-values.local.mjs.\n" +
...
if (!srcPath) {
  console.error('Usage: node buildBmAgm.mjs "<path to source .docx>"');
  process.exit(1);
}
...
const { transformParagraph, mismatches, repaired, debugLines: DEBUG_LINES } = createTransformer({
  corrections: CORRECTIONS,
  globalTokens: GLOBAL_TOKENS,
  paraTokens: PARA_TOKENS,
  debug: !!process.env.BM_DEBUG_DUMP,
});

// The one page-break mechanism that satisfies both Word and docx-preview —
// see createPageBreakStyle() in core.mjs for why the other two do not.
const pageBreak = createPageBreakStyle('BmPageStart', 'BM Page Start');

const { out, count: idx } = walkBody({
  body,
  transformParagraph,
...
// stripHighlight / stripListBullet / swapFonts / scaleFontSizes /
// tightDocDefaults / mangalToNirmala all live in core.mjs — every Preeti
// source needs them. What stays here is only what is specific to THIS
// document: its own alignment repairs and its §51 capital report.
//
// This source has exactly one paragraph (the §92 Declaration page's "no
// other beneficial owner" self-declaration line) formatted as a real Word
// bulleted list item, which is why stripListBullet is applied below.
//
...
// (buildBmAgm.mjs's own structural check above proves it), so this is unambiguous.
...
// The factor is this document's own calibration and stays here;
// scaleFontSizes() in core.mjs is the implementation.
const FONT_SCALE = 0.90;
const scaleBm = s => scaleFontSizes(s, FONT_SCALE);
...
//    the limit.
//    tightDocDefaults() in core.mjs is the implementation; 276 is its default.
...
//    mangalToNirmala() in core.mjs is the implementation.
...
files['word/styles.xml'] = Buffer.from(
  mangalToNirmala(scaleBm(tightDocDefaults(files['word/styles.xml'].toString('utf8')))),
  'utf8'
);
files['word/styles.xml'] = Buffer.from(
...
if (process.env.BM_DEBUG_DUMP) {
  ...
  console.log('debug dump written: tools/registrarDocx/debug-dump.local.txt');
}
