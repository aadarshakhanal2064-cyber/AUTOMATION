...
const TrialBalanceReader = (() => {
  ...
  const BLOCK_TITLES = {
    ...
    expenses: { title: 'B. Expenses', side: 'dr' },
    revenue:  { title: 'C. Revenue', side: 'cr' },
    equity:   { title: 'D. Equity and Liabilities', side: 'cr' },
  };

  function toReport(parsed) {
    if (!parsed || !parsed.ok) return null;
    const blocks = [];
    for (const id of ['assets', 'expenses', 'revenue', 'equity']) {
      ...
      blocks.push({
        ...
        sections: b.sections.map(sec => ({
          id: sec.id,
          // The sheet's own wording, tidied of its numbering, so the page
          // reads like the client's file rather than like our vocabulary.
