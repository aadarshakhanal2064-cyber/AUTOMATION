...
const asScope = WorkflowEngine.createClientScope({
  ...
  load(it) {
    ...
    // caption says so.
    asTaxRule.returnType = NepalTax.returnTypeFromClient(it.it_return_type)
...
function asRenderTaxRule() {
  ...
  const typePicker = `
    <div class="form-group" style="margin:0;">
      ...
      <select onchange="asTaxRuleSet('returnType', this.value)">
  ...
  const capRow = (label, value, cap) => {
    if (cap == null) return `<tr><td>${label}</td><td style="text-align:right; font-variant-numeric:tabular-nums;">${asFmt(value)}</td><td style="text-align:right; color:var(--text-muted);">no limit</td><td></td></tr>`;
    const over = value > cap;
    return `<tr>
      <td>${label}</td>
      <td style="text-align:right; font-variant-numeric:tabular-nums;${over ? ' color:var(--red-dk); font-weight:600;' : ''}">${asFmt(value)}</td>
