...
function handleClientSearch(val) {
  ...
  list.innerHTML = matches.map((c, i) => `
    <div class="autocomplete-item" data-idx="${i}" onmousedown="selectClient('${escHtml(c.name)}','${escHtml(c.email||'')}')">
          ...
          <tr>
            ...
            <td>
              <div class="client-actions">
                ...
                <button class="btn btn-danger btn-sm" onclick="deleteClient('${c.id}', '${escHtml(c.name)}')">Delete</button>
              </div>
            </td>` : ''}
          </tr>
        `).join('')}
